...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function calcularTotal() {
        ...
        const total = (qtdAgua * precoGas) + (qtdVasilhame * precoVasilhame);
    ...
    pedidoForm.addEventListener("submit", function (event) {
        ...
        if (quantidade < 1) {
            alert("Informe a quantidade de botijões.");
            return;
        }
        ...
        if (incluirVasilhame.checked) {
            ...
            if (vasilhameQtd > quantidade) {
                alert("Você não pode pedir mais vasilhames do que botijões.");
                return;
            }
        }

        const total = (quantidade * precoGas) + (vasilhameQtd * precoVasilhame);
      ...
      Quantidade: ${quantidade} botijão(ões)<br>
    ...
    `;
        ...
        document.getElementById("confirmarPedido").onclick = function () {
            const mensagem = `Olá,%20gostaria%20de%20pedir%20botijão%20de%20gás.%0A%0A` +
                `*Nome*: ${nome}%0A` +
                `*Endereço*: ${endereco}%0A` +
                `*Quantidade*: ${quantidade}%20botijão(ões)%0A` +
